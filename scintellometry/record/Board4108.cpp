 ...
 */
  ...
  ioctl(BoardHandle, IOCTL_SETDMATIMEOUT, 60);
...
void AL4108::SetupAcq(int Samples) {
  ...
  ioctl(BoardHandle, IOCTL_ABORTDMA);    // Do we have to abort DMA before fresh run !!!
...
void AL4108::StartDMA(int SizeInBytes) {
  ...
  //DMACNT->Set(SizeInDWords);               // Set the DMA Count
   ...
   DMACNT->Set(0x00FFFFFF);

  //DMACNT->Set(0xFFFFFFFF);  // Set the DMA Count Really high
  //ioctl(BoardHandle, IOCTL_TRANSFERCOUNT, SizeInBytes);
  ioctl(BoardHandle, IOCTL_TRANSFERCOUNT, BufferSize);
...
void AL4108::SetupRegisters(unsigned char *FPGAaddr)
{
  ...
  TWOTHIRD                  = new TReg(FPGAaddr+0x97, eREGSIZE_RBW_BYTE, 0x30, 4);
